...
{
    [TestClass()]
    public class FileEncryptionTesting
    {
        [TestMethod()]
        ...
        private void AssertSaveContacts()
        {
            ...
            byte[] key = ue.GetBytes("hello");
            SHA256 shHash = SHA256.Create();
            byte [] encKey = shHash.ComputeHash(key);
            var actual = FileEncryption.SaveContacts(contacts, Path, encKey);
            // Assert
        ...
        private void AssertGetContacts()
        {
            ...
            SHA256 shHash = SHA256.Create();
            byte[] encKey = shHash.ComputeHash(key);
            var actual = FileEncryption.OpenContacts(Path, encKey);
